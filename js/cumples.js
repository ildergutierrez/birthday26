// MM-DD
export const birthdays = {

  "01-00": [

  ],
  // "01-30": [
  //   { nombre: "Ruby Colmenares", sexo: "F", imagen: "imagenes/user/ruth_colmemenares.jpg" },
    
  // ],
  "01-09": [
    { nombre: "Anyi Maldonado", sexo: "F", imagen: "imagenes/user/" }
  ],

  "01-19": [
    { nombre: "Maria Martinez", sexo: "F", imagen: "imagenes/user/maria.jpg" }
  ],

  "01-26": [
    { nombre: "Luz Camargo", sexo: "F", imagen: "imagenes/user/luz_keren.jpg" },
  ],

  "02-01": [
    { nombre: "Wendy Muñoz", sexo: "F", imagen: "imagenes/user/" }
  ],

  "02-05": [
    { nombre: "Luara Cardenas", sexo: "F", imagen: "imagenes/user/laura_cardenas.jpg" }
  ],
  "02-10": [
    { nombre: "Jhon Montaño", sexo: "M", imagen: "imágenes/user/" }
  ],
  "02-20": [
    { nombre: "Julián Castro", sexo: "M", imagen: "imagenes/user/julian.jpg" }
  ],

  "02-22": [
    { nombre: "Brenda Ramos", sexo: "F", imagen: "imagenes/user/brenda_ramos.jpg" }
  ],
  "03-01": [
    { nombre: "Sandrith Ballesteros", sexo: "F", imagen: "imagenes/user/" }
  ],
  "04-01": [
    { nombre: "Eiduen Contreras", sexo: "M", imagen: "imagenes/user/eiduen_contreras.jpg" }
  ],

  "04-07": [
    { nombre: "Geraldine Trillos", sexo: "F", imagen: "imagenes/user/geral_trillos.jpg" }
  ],

  "04-12": [
    { nombre: "Ilder Gutierrez", sexo: "M", imagen: "imagenes/user/ilder_gutierrez.jpg" }
  ],

  "04-13": [
    { nombre: "Yineth Trillos", sexo: "F", imagen: "imagenes/user/yineth_trillos.jpg" }
  ],

  "04-17": [
    { nombre: "Josue Gabalo", sexo: "M", imagen: "imagenes/user/" }
  ],

  "04-21": [
    { nombre: "Julissa Armesto", sexo: "F", imagen: "imagenes/user/" }
  ],

  "05-01": [
    { nombre: "Wendy Quintero", sexo: "F", imagen: "imagenes/user/wendy_quintero.jpg" }
  ],

  "05-21": [
    { nombre: "Deimer Gutierrez", sexo: "M", imagen: "imagenes/user/deimer_gutierrez.jpg" }
  ],

  "05-30": [
    { nombre: "Lorena Ramirez", sexo: "F", imagen: "imagenes/user/lorena_ramirez.png" }
  ],

  "06-07": [
    { nombre: "Leidy Cardenas", sexo: "F", imagen: "imagenes/user/" }
  ],

  "06-20": [
    { nombre: "Juan Pérez", sexo: "M", imagen: "imagenes/user/juan_perez.png" },
    { nombre: "Tatiana Arias", sexo: "F", imagen: "imagenes/user/tatiana_arias.jpg" }
  ],

  "07-06": [
    { nombre: "Paola Contreras", sexo: "F", imagen: "imagenes/user/paola_contretas.jpg" }
  ],

  "07-14": [
    { nombre: "Kelly Bayona", sexo: "F", imagen: "imagenes/user/kelly_angarita.svg" }
  ],

  "07-25": [
    { nombre: "Jhon Alejandro", sexo: "M", imagen: "imagenes/user/" }
  ],

  "07-28": [
    { nombre: "Katerin Garcia", sexo: "F", imagen: "imagenes/user/" }
  ],

  "08-26": [
    { nombre: "Jhon Martinez", sexo: "M", imagen: "imagenes/user/edinson_martinez.jpg" }
  ],

  "09-05": [
    { nombre: "Antonella Salazar", sexo: "F", imagen: "imagenes/user/antonella_salazar.jpg" }
  ],

  "09-11": [
    { nombre: "Teresa Pernet", sexo: "F", imagen: "imagenes/user/teresa.jpg" }
  ],

  "10-03": [
    { nombre: "Daniela Cardenas", sexo: "F", imagen: "imagenes/user/daniela_cardenas.jpg" }
  ],

  "10-21": [
    { nombre: "Fairon Chávez", sexo: "M", imagen: "imagenes/user/fairon_chavez.jpg" }
  ],
  "11-24": [
    { nombre: "Jeison Jimenez", sexo: "M", imagen: "imagenes/" },
  ],

  "11-07": [
    { nombre: "Yurelis Gutierrez", sexo: "F", imagen: "imagenes/user/yurelis.jpg" }
  ],

  "11-19": [
    { nombre: "Yulieth Cuadros", sexo: "F", imagen: "imagenes/user/" }
  ],

  "11-24": [
    { nombre: "Jeison Quintero", sexo: "M", imagen: "imagenes/user/jeixon_quintero.jpg" }
  ],

  "12-09": [
    { nombre: "Andres Nuñez", sexo: "M", imagen: "imagenes/user/" }
  ],

  "12-23": [
    { nombre: "Nayobis Chavez", sexo: "F", imagen: "imagenes/user/nayobis.jpg" }
  ],

  "12-26": [
    { nombre: "Marisol Carmona", sexo: "F", imagen: "imagenes/user/marisol_carmona.jpg" }
  ],

  "12-30": [
    { nombre: "Chaida Duran", sexo: "F", imagen: "imagenes/user/" }
  ],

  "12-31": [
    { nombre: "Karol Vargas", sexo: "F", imagen: "imagenes/user/karol_vargas.jpg" }
  ]

};
